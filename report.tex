...
\mypar{Related work} There is a paper from Püschel et al. which explains how to do tiling and vectorization of the Floyd-Warshall algorithm, which also we use in the transitive closure. However, since this work is done on general graphs and our work is done on the special case where all edges have equal length, there is considerable amount of optimization. There is also an excellent manual from Intel, Intel® 64 and IA-32 Architectures Optimization Reference Manual, which has proven very useful for us. 
...
algorithm \cite{Floyd1962} with logical operations – 1 AND and 1 OR in the innermost 
loop, thus the exact number of logical operations is \(2n^3\). We do not count 
...
\erik{The cost analysis need to be more complete. We need to calculate our operations intensity and also take the other code in account. This will allow us to say if we are memory bound or not, something I said we are but we need to also prove it here.}
...
essence of it is to call FWabc whenever possible (3 different submatrices) 
during the processing of smaller submatrices. We observed about 4x improvement 
compared to naïve DM for large n (there is no big difference on small n, because 
...
starting point and considering the number of available registers in an 
x86-64 processor, we implemented hand-written SSA basic blocks using 
scalar replacement to avoid pointer aliasing. We strived
to utilize the ideas found in micro-MMM. However both in FW and FWabc 
there is a low degree of register reuse and the results showed similar 
or even a bit lower performance when using the optimized basic blocks.

\erik{TODO: Add bit-packing}


\section{Experimental Results}\label{sec:exp}
\erik{I will write this}
